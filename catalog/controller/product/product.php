				...
				foreach ($qtyDetail as $key => $value) {
					if (count($value['images']) > 0) {
						foreach ($value['images'] as $imgKey => $img) {
							$qtyDetail[$key]['images'][$imgKey]['url'] = $this->model_tool_image->resize($img['name'], 500,650);
			...
			foreach ($results as $result) {
				...
				if (count($quantityDetail) > 0) {
					foreach ($quantityDetail as $key => $value) {
						if (count($value['images']) > 0) {
							foreach ($value['images'] as $imgKey => $img) {
								$quantityDetail[$key]['images'][$imgKey]['url'] = $this->model_tool_image->resize($img['name'], 500, 650);
							}
						}
					}
